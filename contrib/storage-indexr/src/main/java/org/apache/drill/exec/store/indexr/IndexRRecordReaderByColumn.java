...
public class IndexRRecordReaderByColumn extends IndexRRecordReader {
    ...
    public int next() {
        ...
        while (curPackId < packRSResults.length && rowCount < TARGET_RECORD_COUNT) {
            rowCount += processPack(curPackId, rowCount);
            curPackId++;
        }
        return rowCount;
    }

    private int processPack(int packId, int preRowCount) {
        byte rsRes = packRSResults[packId];
        if (rsRes == RSValue.None) {
            return 0;
        }
        // Only filter none pack for now.
        // TODO handle All case.
        DataPack[] dataPacks = new DataPack[columns.length];
        for (int i = 0; i < columns.length; i++) {
            dataPacks[i] = columns[i].pack(packId);
        }
        int rowIdInPack = 0;
        for (ProjectedColumnInfo info : projectedColumnInfos) {
            switch (info.dataType) {
                case ColumnType.INT:
                    ((IntVector.Mutator) info.valueVector.getMutator()).setSafe(preRowCount + rowIdInPack, dataPacks[info.ordinal].intValueAt(rowIdInPack));
                    break;
                case ColumnType.LONG:
                    ((BigIntVector.Mutator) info.valueVector.getMutator()).setSafe(preRowCount + rowIdInPack, dataPacks[info.ordinal].longValueAt(rowIdInPack));
                    break;
                case ColumnType.FLOAT:
                    ((Float4Vector.Mutator) info.valueVector.getMutator()).setSafe(preRowCount + rowIdInPack, dataPacks[info.ordinal].floatValueAt(rowIdInPack));
                    break;
                case ColumnType.DOUBLE:
                    ((Float8Vector.Mutator) info.valueVector.getMutator()).setSafe(preRowCount + rowIdInPack, dataPacks[info.ordinal].doubleValueAt(rowIdInPack));
                    break;
                case ColumnType.STRING:
                    info.column.rawValueAt(rowIdInPack, bytePiece);
